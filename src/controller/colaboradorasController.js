...
const putColaboradora = (req, res) => {
  const id = req.params.id;
  const colaboradoraASerModificada = colaboradoras.find((colaboradora) => colaboradora.id == id);
  const colaboradoraAtualizada = req.body;
  const index = colaboradoras.indexOf(colaboradoraASerModificada);
  colaboradoraASerModificada.nomeColaboradora = colaboradoraAtualizada.nomeColaboradora;

  colaboradoras.splice(index, 1, colaboradoraASerModificada);
